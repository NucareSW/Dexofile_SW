...
namespace {
QString spectrumLogPath() {
    return ComponentManager::instance().dataDir() + "/NDT_Specs.text";
}

void appendSpectrumLog(const std::shared_ptr<Spectrum>& spc) {
    if (!spc) return;

    const auto dirPath = ComponentManager::instance().dataDir();
    QDir().mkpath(dirPath);

    QFile f(spectrumLogPath());
    if (!f.open(QIODevice::WriteOnly | QIODevice::Append | QIODevice::Text)) {
        nucare::logW() << "Failed to open spectrum log file:" << spectrumLogPath();
        return;
    }
